...
from bart_intrinsic import MyBart_intrinsic
from bartPrompt import MyBartPrompt_AE, MyBartPrompt_ensemble
from utils import freeze_embeds, freeze_embeds_adapter, trim_batch, get_tasks_list
...
def get_params_for_adapter_optimization(module: torch.nn.Module, AE_recover = False, AE_recover_stage_two = False):
    params = []
    params_optimized = []

    for n, p in module.named_parameters():
        if "intrinsic" in n or "adapter" in n:
            if AE_recover and "adapter" in n:
                continue
            params_optimized.append(n)
            params.append({'params': [p], 'lr': 1e-4 if "intrinsic" in n else 3e-4})
            print(n)
    # if torch.distributed.get_rank() == 0:
    #     print("print params", params)
    return params
...
def save_ckpt(model, args, global_step, scheduler, optimizer, best_accuracy, AE_recover, logger, prefix=None):
    ...
    if AE_recover:
        ...
        elif args.do_adapter:
            model_state_dict = {k:v.cpu() for (k, v) in model.state_dict().items() if 'intrinsic' in k}
    else:
        if args.do_prompt:
            model_state_dict = {k:v.cpu() for (k, v) in model.state_dict().items() if 'prompt' in k}
        elif args.do_adapter:
            model_state_dict = {k:v.cpu() for (k, v) in model.state_dict().items() if 'adapter' in k}
    ckpt_to_save = {
    ...
    if args.do_train:
        if args.checkpoint is not None:
            ...
            if args.do_prompt:
                ...
                model = MyBart_intrinsic.from_pretrained(args.model, intrinsic_dim = args.intrinsic_dim)
            else:
                model = MyBart.from_pretrained(args.model)
        ...
        if args.freeze_embeds:
            ...
            if args.do_adapter:
                freeze_embeds_adapter(model, AE_recover=args.AE_recover, AE_recover_stage_two=args.AE_recover_stage_two)
            else:
        ...
        if args.do_prompt:
            ...
            optimizer_grouped_parameters = get_params_for_adapter_optimization(model, AE_recover = args.AE_recover, AE_recover_stage_two = args.AE_recover_stage_two)
        else:
...
def train(args, logger, model, train_data, dev_data, test_data, optimizer, scheduler):
    ...
    global_step = 0
    train_losses = []
    if args.do_AE and args.do_prompt:
        train_losses_distil = []
        train_losses_AE = []
    best_accuracy = -1.0
    wait_step = 0
