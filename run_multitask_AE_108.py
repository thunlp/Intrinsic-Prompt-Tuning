...
def get_params_for_prompt_optimization(module: torch.nn.Module, AE_recover = False):
    params = []
    params_optimized = []
    for t in module.named_modules():
        if "prompt" in t[0]:
            if AE_recover and "prompt_task" not in t[0]:
                continue
...
def run(args, logger, recover_task=None):
    ...
    if args.do_train:
        if args.checkpoint is not None:
            ...
            if args.do_prompt:
                if args.do_AE:
                    model = MyBartPrompt_AE.from_pretrained(args.model, prompt_num=args.prompt_num, intrinsic_dim=args.intrinsic_dim, AE_loss=args.AE_loss, Distil_loss=args.Distil_loss, alpha_AE=args.alpha_AE, AE_type=args.AE_type, AE_recover=args.AE_recover)
        ...
        if args.freeze_embeds:
            ...
            freeze_embeds(model, AE_recover=args.AE_recover)
        ...
        if args.do_prompt:
            optimizer_grouped_parameters = get_params_for_prompt_optimization(model, AE_recover = args.AE_recover)
        else:
