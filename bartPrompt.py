...
class MyBartPrompt_AE(BartForConditionalGeneration):
    def __init__(self, config: BartConfig, prompt_num=100, intrinsic_dim=10, AE_loss=1, Distil_loss=1, alpha_AE=200, AE_type=0, AE_recover=False,AE_recover_stage_two=False):
        super().__init__(config)
        self.prompt_num = prompt_num
        ...
        self.AE_recover_stage_two = AE_recover_stage_two
        if self.AE_recover_stage_two:
            assert self.AE_recover
        if self.AE_type == 0:
    ...
    def forward(self, input_ids, attention_mask=None, encoder_outputs=None,
        ...
        if task_prompt_recored is None:
            if self.AE_recover and not self.AE_recover_stage_two:
                task_prompt_recored = self.generate_prompt_recover(task_prompt, input_ids.size()[0])
            elif self.AE_recover_stage_two:
