...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens, prompt_num):
        super().__init__()
        ...
        embed_dim = embed_tokens.embedding_dim #768
        self.embed_scale = math.sqrt(embed_dim) if config.scale_embedding else 1.0
        self.padding_idx = embed_tokens.padding_idx
        self.max_source_positions = config.max_position_embeddings

        self.embed_tokens = embed_tokens
        self.prompt_embeddings = nn.Embedding(prompt_num, embed_dim)
    ...
    def init_prompt_emb(self, init_ids):
        prompt_weights = self.embed_tokens(init_ids).detach()
        self.prompt_embeddings.weight.data = prompt_weights

    def forward(
        self, input_ids, attention_mask=None, task_prompt_recored=None,
    ):
        """
        Args:
        ...
        inputs_embeds = self.embed_tokens(input_ids * (input_ids >= 0).int()) * (input_ids >= 0).int().unsqueeze(2) * self.embed_scale
        embed_pos = self.embed_positions(input_ids)

        prompt_ids = - (input_ids + 1)
        if task_prompt_recored == None:
            ...
            prompt_emb = task_prompt_recored * (prompt_ids >= 0).int().unsqueeze(2)

        x = inputs_embeds + embed_pos + prompt_emb
...
class BartModel(PretrainedBartModel):
    def __init__(self, config: BartConfig, prompt_num):
        ...
        self.encoder = BartEncoder(config, self.shared, prompt_num)
    ...
    def forward(
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(input_ids=input_ids, attention_mask=attention_mask, task_prompt_recored=task_prompt_recored)
...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    def __init__(self, config: BartConfig, prompt_num=100):
        super().__init__(config)
        base_model = BartModel(config, prompt_num)
