...
class EncoderLayer(nn.Module):
    def __init__(self, config: BartConfig, share_intrinsic=None, intrinsic_dim=10):
        super().__init__()
        ...
        self.adapter_att = IntrinsicAdapter(self.intrinsic_dim, config.d_model, 16, 'swish', share_intrinsic=share_intrinsic)

        self.adapter_ffn = IntrinsicAdapter(self.intrinsic_dim, config.d_model, 16, 'swish', share_intrinsic=share_intrinsic)
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        residual_adapter_att = x
        adaptered_states_att = self.adapter_att(x, residual=residual_adapter_att)

        x = F.dropout(adaptered_states_att, p=self.dropout, training=self.training)
        x = residual + x
        ...
        residual_adapter_ffn = x
        adaptered_states_ffn = self.adapter_ffn(x, residual=residual_adapter_ffn)

        x = residual + adaptered_states_ffn
        if not self.normalize_before:
            x = self.final_layer_norm(x)
...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens, share_intrinsic=None, intrinsic_dim=10):
        super().__init__()
        ...
        self.layers = nn.ModuleList([EncoderLayer(config, share_intrinsic, intrinsic_dim) for _ in range(config.encoder_layers)])
...
class BartDecoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding, share_intrinsic=None, intrinsic_dim=10):
        super().__init__()
        self.output_attentions = config.output_attentions
        self.output_hidden_states = config.output_hidden_states
        self.dropout = config.dropout
        ...
        self.layers = nn.ModuleList(
            [DecoderLayer(config, share_intrinsic, intrinsic_dim) for _ in range(config.decoder_layers)]
...
class BartModel(PretrainedBartModel):
    def __init__(self, config: BartConfig, intrinsic_dim = 10):
        ...
        self.share_intrinsic = nn.Parameter(torch.zeros(self.intrinsic_dim, 1))
        self.share_intrinsic.data.normal_(mean=0, std=0.02)

        self.encoder = BartEncoder(config, self.shared, share_intrinsic=self.share_intrinsic, intrinsic_dim=self.intrinsic_dim)
        self.decoder = BartDecoder(config, self.shared, share_intrinsic=self.share_intrinsic, intrinsic_dim=self.intrinsic_dim)
...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    def __init__(self, config: BartConfig, intrinsic_dim = 10):
        super().__init__(config)
        base_model = BartModel(config, intrinsic_dim = intrinsic_dim)
