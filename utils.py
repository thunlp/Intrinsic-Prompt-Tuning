...
def freeze_embeds_adapter(model, AE_recover=False, AE_recover_stage_two=False):
    """Freeze token embeddings and positional embeddings for bart, just token embeddings for t5."""

    def freeze_params(model: nn.Module):
        """Set requires_grad=False for each of model.parameters()"""
        for n, par in model.named_parameters():
            if 'adapter' in n or 'model.share_intrinsic' in n:
                continue
            par.requires_grad = False

    if AE_recover:
        for n, p in model.named_parameters():
            if 'model.share_intrinsic' not in n:
                p.requires_grad = False
    else:
        freeze_params(model.model)
